 ...
 */
public final class Realtime4Queue<T> implements PQueue<T> {
	@SuppressWarnings("rawtypes")
	private static final Realtime4Queue<?> EMPTY = new Realtime4Queue();

	@SuppressWarnings("unchecked")
	public static <T> Realtime4Queue<T> create() {
		return (Realtime4Queue<T>) EMPTY;
	}
	...
	private Realtime4Queue() {
		this(PCollections.emptyStack(), PCollections.emptyStack(), null, null, null, null, 0);
	}

	private Realtime4Queue(PStack<T> head, PStack<T> tail, PStack<T> tailReverseFrom, PStack<T> tailReverseTo,
	 ...
	 */
	private static boolean needsStep(final Realtime4Queue<?> q) {
	 ...
	 */
	private static <T> Realtime4Queue<T> step(final Realtime4Queue<T> q) {
		...
		return new Realtime4Queue<>(head, tail, tailReverseFrom, tailReverseTo, headReverseFrom, headReverseTo,
				headCopied);
	}
	...
	private static <T> PQueue<T> create(PStack<T> head, PStack<T> tail, PStack<T> tailReverseFrom,
		...
		Realtime4Queue<T> ret = new Realtime4Queue<>(head, tail, tailReverseFrom, tailReverseTo, headReverseFrom,
